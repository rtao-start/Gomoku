...
def negamax(is_ai, depth, alpha, beta):
    ...
    for next_step in blank_list:
        ...
        if value > alpha:
            # alpha 是当前搜索路径中，极大化玩家可以保证的最小值（即最大化玩家的最佳值的下界）。
...
def main():
    ...
    while g == 0:
        # g 用于控制游戏的状态，当 g 变为非零时，游戏结束。

        if change % 2 == 1: 
            # 检查 change 变量，如果 change 是奇数，则轮到玩家1（AI）进行操作
            pos = ai()  # AI生成落子，该函数返回一个坐标元组 pos

            if pos in list3:
                # 检查 pos 是否已经在 list3 中，list3 存储了所有已占用的位置
                message = Text(Point(200, 200), "不可用的位置" + str(pos[0]) + "," + str(pos[1]))
                message.draw(win)
                g = 1

            list1.append(pos) # AI的全部落子
            list3.append(pos) # 棋盘上的全部落子
...
def move():
    url = "http://192.168.220.68:13200"
    current_player = 1
    # -----获取棋盘状态------
    board = requests.get(url+'/board')
    data = board.json()
    board = data['board']  # 19x19的棋盘，0表示空，1表示玩家1，2表示玩家2
